	...
	async getResponse(response, url, headers, outputType) {
		...
		if (typeof response == "object") {
			...
			if ("output" in response && Array.isArray(response.output)) {
				const blob = await (await fetch(response.output[0])).blob();
...
var HFInferenceTextGenerationTask = class extends HFInferenceTask {
	async getResponse(response) {
		const res = toArray(response);
		if (Array.isArray(res) && res.every((x) => "generated_text" in x && typeof x?.generated_text === "string")) return res?.[0];
...
async function resolveProvider(provider, modelId, endpointUrl) {
	...
	if (provider === "auto") {
		...
		provider = (await fetchInferenceProviderMappingForModel(modelId))[0]?.provider;
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/clarifai.js
/**
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/black-forest-labs.js
/**
...
var BlackForestLabsTextToImageTask = class extends TaskProviderHelper {
	...
	prepareHeaders(params, binary) {
		const headers = { Authorization: params.authMethod !== "provider-key" ? `Bearer ${params.accessToken}` : `X-Key ${params.accessToken}` };
		if (!binary) headers["Content-Type"] = "application/json";
		return headers;
	}
	makeRoute(params) {
		if (!params) throw new InferenceClientInputError("Params are required");
	...
	async getResponse(response, url, headers, outputType) {
		...
		for (let step = 0; step < 5; step++) {
			...
			if (!resp.ok) throw new InferenceClientProviderApiError("Failed to fetch result from black forest labs API", {
				url: urlObj.toString(),
				method: "GET",
			...
			if (typeof payload === "object" && payload && "status" in payload && typeof payload.status === "string" && payload.status === "Ready" && "result" in payload && typeof payload.result === "object" && payload.result && "sample" in payload.result && typeof payload.result.sample === "string") {
				if (outputType === "json") return payload.result;
				if (outputType === "url") return payload.result.sample;
				return await (await fetch(payload.result.sample)).blob();
			}
		}
...
* Thanks!
*/
var FAL_AI_SUPPORTED_BLOB_TYPES = [
	"audio/mpeg",
	"audio/mp4",
	"audio/wav",
	"audio/x-wav"
...
var FalAITextToImageTask = class extends FalAiQueueTask {
	...
	async getResponse(response, url, headers, outputType) {
		...
		if (typeof result === "object" && "images" in result && Array.isArray(result.images) && result.images.length > 0 && "url" in result.images[0] && typeof result.images[0].url === "string" && isUrl(result.images[0].url)) {
			...
			const blob = await (await fetch(result.images[0].url)).blob();
			return outputType === "dataUrl" ? dataUrlFromBlob(blob) : blob;
		}
...
var FalAIImageToImageTask = class extends FalAiQueueTask {
	...
	async getResponse(response, url, headers) {
		const result = await this.getResponseFromQueueApi(response, url, headers);
		if (typeof result === "object" && !!result && "images" in result && Array.isArray(result.images) && result.images.length > 0 && typeof result.images[0] === "object" && !!result.images[0] && "url" in result.images[0] && typeof result.images[0].url === "string" && isUrl(result.images[0].url)) return await (await fetch(result.images[0].url)).blob();
...
var FalAITextToVideoTask = class extends FalAiQueueTask {
	...
	async getResponse(response, url, headers) {
		const result = await this.getResponseFromQueueApi(response, url, headers);
		if (typeof result === "object" && !!result && "video" in result && typeof result.video === "object" && !!result.video && "url" in result.video && typeof result.video.url === "string" && isUrl(result.video.url)) return await (await fetch(result.video.url)).blob();
...
var FalAIImageToVideoTask = class extends FalAiQueueTask {
	...
	async getResponse(response, url, headers) {
		const result = await this.getResponseFromQueueApi(response, url, headers);
		if (typeof result === "object" && result !== null && "video" in result && typeof result.video === "object" && result.video !== null && "url" in result.video && typeof result.video.url === "string" && "url" in result.video && isUrl(result.video.url)) return await (await fetch(result.video.url)).blob();
...
var FalAIAutomaticSpeechRecognitionTask = class extends FalAITask {
	...
	async getResponse(response) {
		const res = response;
		if (typeof res?.text !== "string") throw new InferenceClientProviderOutputError(`Received malformed response from Fal.ai Automatic Speech Recognition API: expected { text: string } format, got instead: ${JSON.stringify(response)}`);
	...
	async preparePayloadAsync(args) {
		...
		if (!FAL_AI_SUPPORTED_BLOB_TYPES.includes(contentType)) throw new InferenceClientInputError(`Provider fal-ai does not support blob type ${contentType} - supported content types are: ${FAL_AI_SUPPORTED_BLOB_TYPES.join(", ")}`);
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/hyperbolic.js
...
var HyperbolicTextGenerationTask = class extends BaseTextGenerationTask {
	...
	async getResponse(response) {
		if (typeof response === "object" && "choices" in response && Array.isArray(response?.choices) && typeof response?.model === "string") return { generated_text: response.choices[0].message.content };
...
var HyperbolicTextToImageTask = class extends TaskProviderHelper {
	...
	preparePayload(params) {
		if (params.outputType === "url") throw new InferenceClientInputError("hyperbolic provider does not support URL output. Use outputType 'blob', 'dataUrl' or 'json' instead.");
		return {
			...omit(params.args, ["inputs", "parameters"]),
			...params.args.parameters,
			prompt: params.args.inputs,
	...
	async getResponse(response, url, headers, outputType) {
		if (typeof response === "object" && "images" in response && Array.isArray(response.images) && response.images[0] && typeof response.images[0].image === "string") {
			if (outputType === "json") return { ...response };
			if (outputType === "dataUrl") return `data:image/jpeg;base64,${response.images[0].image}`;
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/nebius.js
...
var NebiusConversationalTask = class extends BaseConversationalTask {
	...
	preparePayload(params) {
		...
		if (responseFormat?.type === "json_schema" && responseFormat.json_schema?.schema) payload["guided_json"] = responseFormat.json_schema.schema;
		return payload;
	}
};
var NebiusTextGenerationTask = class extends BaseTextGenerationTask {
	...
	async getResponse(response) {
		if (typeof response === "object" && "choices" in response && Array.isArray(response?.choices) && response.choices.length > 0 && typeof response.choices[0]?.text === "string") return { generated_text: response.choices[0].text };
...
var NovitaTextToVideoTask = class extends TaskProviderHelper {
	...
	async getResponse(response, url, headers) {
		...
		if (typeof taskResult === "object" && !!taskResult && "videos" in taskResult && typeof taskResult.videos === "object" && !!taskResult.videos && Array.isArray(taskResult.videos) && taskResult.videos.length > 0 && "video_url" in taskResult.videos[0] && typeof taskResult.videos[0].video_url === "string" && isUrl(taskResult.videos[0].video_url)) return await (await fetch(taskResult.videos[0].video_url)).blob();
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/nvidia.js
/**
...
* Thanks!
*/
var NvidiaConversationalTask = class extends BaseConversationalTask {
	constructor() {
...
var ReplicateTextToImageTask = class extends ReplicateTask {
	...
	async getResponse(res, url, headers, outputType) {
		if (typeof res === "object" && "output" in res && typeof res.output === "string" && isUrl(res.output)) {
			...
			const blob = await (await fetch(res.output)).blob();
			return outputType === "dataUrl" ? dataUrlFromBlob(blob) : blob;
		}
		if (typeof res === "object" && "output" in res && Array.isArray(res.output) && res.output.length > 0 && typeof res.output[0] === "string") {
			...
			const blob = await (await fetch(res.output[0])).blob();
			return outputType === "dataUrl" ? dataUrlFromBlob(blob) : blob;
		}
	...
	async getResponse(response) {
		...
		if (response && typeof response === "object") {
			if ("output" in response) {
				if (typeof response.output === "string") return await (await fetch(response.output)).blob();
				else if (Array.isArray(response.output)) return await (await fetch(response.output[0])).blob();
			}
		}
...
var ReplicateTextToVideoTask = class extends ReplicateTask {
	async getResponse(response) {
		if (typeof response === "object" && !!response && "output" in response && typeof response.output === "string" && isUrl(response.output)) return await (await fetch(response.output)).blob();
...
var ReplicateAutomaticSpeechRecognitionTask = class extends ReplicateTask {
	...
	async getResponse(response) {
		...
		if (out && typeof out === "object") {
			...
			if (typeof out.txt_file === "string") return { text: await (await fetch(out.txt_file)).text() };
		}
...
var ReplicateImageToImageTask = class extends ReplicateTask {
	...
	async getResponse(response) {
		if (typeof response === "object" && !!response && "output" in response && Array.isArray(response.output) && response.output.length > 0 && typeof response.output[0] === "string") return await (await fetch(response.output[0])).blob();
		if (typeof response === "object" && !!response && "output" in response && typeof response.output === "string" && isUrl(response.output)) return await (await fetch(response.output)).blob();
		throw new InferenceClientProviderOutputError("Received malformed response from Replicate image-to-image API");
	}
};
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/providers/sambanova.js
var SambanovaConversationalTask = class extends BaseConversationalTask {
	...
	preparePayload(params) {
		...
		if (responseFormat?.type === "json_schema" && responseFormat.json_schema) {
			if (responseFormat.json_schema.strict ?? true) responseFormat.json_schema.strict = false;
		}
		return super.preparePayload(params);
	}
};
var SambanovaFeatureExtractionTask = class extends TaskProviderHelper {
	...
	async getResponse(response) {
		if (typeof response === "object" && "data" in response && Array.isArray(response.data)) return response.data.map((item) => item.embedding);
...
var ZaiImageToTextTask = class extends ZaiTask {
	...
	async preparePayloadAsync(args) {
		const blob = "data" in args && args.data instanceof Blob ? args.data : "inputs" in args ? typeof args.inputs === "string" && isUrl(args.inputs) ? await fetch(args.inputs).then((r) => r.blob()) : args.inputs instanceof Blob ? args.inputs : void 0 : void 0;
...
var PROVIDERS = {
	...
	clarifai: { conversational: new ClarifaiConversationalTask() },
	...
	nvidia: { conversational: new NvidiaConversationalTask() },
...
async function request(args, options) {
	...
	return (await innerRequest(args, getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), options?.task), options)).data;
}
//#endregion
...
async function* streamingRequest(args, options) {
	...
	yield* innerStreamingRequest(args, getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), options?.task), options);
}
//#endregion
...
async function audioClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "audio-classification");
...
async function audioToAudio(args, options) {
	...
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, model), "audio-to-audio");
	const { data: res } = await innerRequest(preparePayload$2(args), providerHelper, {
		...options,
		task: "audio-to-audio"
	});
...
async function automaticSpeechRecognition(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "automatic-speech-recognition");
...
async function textToSpeech(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-to-speech");
...
async function imageClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-classification");
...
async function imageSegmentation(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-segmentation");
	const { data: res } = await innerRequest(await providerHelper.preparePayloadAsync(args), providerHelper, {
		...options,
...
async function imageToImage(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-to-image");
	const { data: res } = await innerRequest(await providerHelper.preparePayloadAsync(args), providerHelper, {
		...options,
		task: "image-to-image"
	});
...
async function imageToText(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-to-text");
	const { data: res } = await innerRequest(await providerHelper.preparePayloadAsync(args), providerHelper, {
		...options,
		task: "image-to-text"
	});
...
async function imageToVideo(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-to-video");
	const { data: res } = await innerRequest(await providerHelper.preparePayloadAsync(args), providerHelper, {
		...options,
		task: "image-to-video"
	});
...
async function imageTextToImage(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-text-to-image");
...
async function imageTextToVideo(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "image-text-to-video");
	const { data: res, requestContext } = await innerRequest(await providerHelper.preparePayloadAsync(args), providerHelper, {
		...options,
...
async function objectDetection(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "object-detection");
	const { data: res } = await innerRequest(preparePayload$1(args), providerHelper, {
		...options,
...
async function textToImage(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-to-image");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
		task: "text-to-image"
	});
...
async function textToVideo(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-to-video");
...
async function zeroShotImageClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "zero-shot-image-classification");
...
async function chatCompletion(args, options) {
	let providerHelper;
	if (args.endpointUrl) providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "conversational");
	else if (!args.provider || args.provider === "auto") providerHelper = new AutoRouterConversationalTask();
	else providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "conversational");
	const { data: response } = await innerRequest(args, providerHelper, {
		...options,
		task: "conversational"
	});
...
async function featureExtraction(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "feature-extraction");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function fillMask(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "fill-mask");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
		task: "fill-mask"
	});
...
async function questionAnswering(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "question-answering");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function sentenceSimilarity(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "sentence-similarity");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function summarization(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "summarization");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
		task: "summarization"
	});
...
async function tableQuestionAnswering(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "table-question-answering");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function textClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-classification");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function textGeneration(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-generation");
	const { data: response } = await innerRequest(args, providerHelper, {
		...options,
...
async function* textGenerationStream(args, options) {
	yield* innerStreamingRequest(args, getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "text-generation"), {
		...options,
		task: "text-generation"
	});
}
...
async function tokenClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "token-classification");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function translation(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "translation");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
		task: "translation"
	});
...
async function zeroShotClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "zero-shot-classification");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function documentQuestionAnswering(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "document-question-answering");
...
async function visualQuestionAnswering(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "visual-question-answering");
	const { data: res } = await innerRequest({
		...args,
		inputs: {
			question: args.inputs.question,
...
async function tabularClassification(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "tabular-classification");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
async function tabularRegression(args, options) {
	const providerHelper = getProviderHelper(await resolveProvider(args.provider, args.model, args.endpointUrl), "tabular-regression");
	const { data: res } = await innerRequest(args, providerHelper, {
		...options,
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/types.js
...
function formatExpression(node, parentPrec = -1) {
	switch (node.type) {
		...
		case "MemberExpression": {
			...
			if (!n.computed && n.property.type !== "Identifier") prop = `(${prop})`;
...
var Template = class {
	...
	*/
	constructor(template) {
		this.parsed = parse(tokenize(template, {
			lstrip_blocks: true,
			trim_blocks: true
		}));
	}
...
new Map([
	...
	["translation", ["بیشتر مساحت جنگل‌های آمازون در حوضه آبریز رود آمازون و ۱۱۰۰ شاخه آن واقع شده‌است.", "مردمان نَبَطی از هزاره‌های یکم و دوم پیش از میلاد در این منطقه زندگی می‌کردند."]],
	["summarization", [[
	...
	["fill-mask", [`زندگی یک سوال است و این که چگونه <mask> کنیم پاسخ این سوال!`, `زندگی از مرگ پرسید: چرا همه من را <mask> دارند اما از تو متنفرند؟`]]
]);
//#endregion
...
var inputsTextGeneration = (model) => {
	if (model.tags.includes("conversational")) return model.pipeline_tag === "text-generation" ? [{
		role: "user",
...
var transformers = (model) => {
	...
	} else autoSnippet.push("# Load model directly", `from transformers import ${info.auto_model}`, `model = ${info.auto_model}.from_pretrained("${model.id}"` + remote_code_snippet + ", dtype=\"auto\")");
...
//#endregion
//#region node_modules/@huggingface/tasks/dist/esm/snippets/types.js
var inferenceSnippetLanguages = [
	"python",
	"js",
	"sh"
];
...
var snippetGenerator = (templateName, inputPreparationFn) => {
	return (model, provider, inferenceProviderMapping, opts) => {
		...
		if (typeof bodyAsObj === "string") try {
		...
		return inferenceSnippetLanguages.map((language) => {
			return (clients[language] ?? []).map((client) => {
				if (!hasTemplate(language, client, templateName)) return;
				const template = loadTemplate(language, client, templateName);
				if (client === "huggingface_hub" && templateName.includes("basic")) {
					if (!(model.pipeline_tag && model.pipeline_tag in HF_PYTHON_METHODS)) return;
					params["methodName"] = HF_PYTHON_METHODS[model.pipeline_tag];
				}
				if (client === "huggingface.js" && templateName.includes("basic")) {
					if (!(model.pipeline_tag && model.pipeline_tag in HF_JS_METHODS)) return;
					params["methodName"] = HF_JS_METHODS[model.pipeline_tag];
				}
				let snippet = template(params).trim();
				if (!snippet) return;
				if (client === "huggingface_hub") snippet = `${snippetImportPythonInferenceClient({ ...params })}\n\n${snippet}`;
				else if (client === "requests") snippet = `${snippetImportRequests({
					...params,
					importBase64: snippet.includes("base64"),
					importJson: snippet.includes("json.")
				})}\n\n${snippet}`;
				if (snippet.includes(placeholder)) snippet = replaceAccessTokenPlaceholder(opts?.directRequest, placeholder, snippet, language, provider, opts?.endpointUrl);
				return {
					language,
					client,
					content: snippet
...
function formatBody(obj, format) {
	switch (format) {
		case "curl": return indentString(formatBody(obj, "json"));
		case "json": return JSON.stringify(obj, null, 4).split("\n").slice(1, -1).join("\n");
		case "python": return indentString(Object.entries(obj).map(([key, value]) => {
			return `${key}=${JSON.stringify(value, null, 4).replace(/"/g, "\"")},`;
		}).join("\n"));
...
function replaceAccessTokenPlaceholder(directRequest, placeholder, snippet, language, provider, endpointUrl) {
	const accessTokenEnvVar = !endpointUrl && (provider == "hf-inference" || !directRequest && (snippet.includes("InferenceClient") || snippet.includes("https://router.huggingface.co"))) ? "HF_TOKEN" : endpointUrl ? "API_TOKEN" : provider.toUpperCase().replace("-", "_") + "_API_KEY";
	if (language === "sh") snippet = snippet.replace(`'Authorization: Bearer ${placeholder}'`, `"Authorization: Bearer $${accessTokenEnvVar}"`);
...
//#endregion
//#region node_modules/@huggingface/inference/dist/esm/snippets/index.js
var snippets_exports = /* @__PURE__ */ __exportAll({ getInferenceSnippets: () => getInferenceSnippets });
//#endregion
export { HfInference, INFERENCE_PROVIDERS, InferenceClient, InferenceClientEndpoint, InferenceClientError, InferenceClientHubApiError, InferenceClientInputError, InferenceClientProviderApiError, InferenceClientProviderOutputError, InferenceClientRoutingError, PROVIDERS, PROVIDERS_HUB_ORGS, PROVIDERS_OR_POLICIES, audioClassification, audioToAudio, automaticSpeechRecognition, chatCompletion, chatCompletionStream, documentQuestionAnswering, featureExtraction, fillMask, getProviderHelper, imageClassification, imageSegmentation, imageTextToImage, imageTextToVideo, imageToImage, imageToText, imageToVideo, makeRequestOptions, makeRequestOptionsFromResolvedModel, objectDetection, questionAnswering, request, sentenceSimilarity, setLogger, snippets_exports as snippets, streamingRequest, summarization, tableQuestionAnswering, tabularClassification, tabularRegression, textClassification, textGeneration, textGenerationStream, textToImage, textToSpeech, textToVideo, tokenClassification, translation, visualQuestionAnswering, zeroShotClassification, zeroShotImageClassification };
